SUBROUTINE Gill4(dt,dtn)
    ...
    real(kind=RKIND)                   :: phi4_norm2,R1R2,R1R3,R2R3,R2R4,R3R4
    ...
    wu3 = wu - 0.5d0*(dsqrt(2.d0)-1.d0)*dt*Lwu1 - (1.d0-dsqrt(2.d0)*0.5d0)*dt*Lwu2
    wh3 = wh - 0.5d0*(dsqrt(2.d0)-1.d0)*dt*Lwh1 - (1.d0-dsqrt(2.d0)*0.5d0)*dt*Lwh2
    
    call L_operator(wu3,wh3,wh_s,Lwu3,Lwh3)
    
    wu4 = wu + 0.5d0*dsqrt(2.d0)*dt*Lwu2 - (1.d0+dsqrt(2.d0)*0.5d0)*dt*Lwu3
    wh4 = wh + 0.5d0*dsqrt(2.d0)*dt*Lwh2 - (1.d0+dsqrt(2.d0)*0.5d0)*dt*Lwh3
    
    call L_operator(wu4,wh4,wh_s,Lwu4,Lwh4)
    
    phi4_wu = (Lwu1+(2.d0-dsqrt(2.d0))*Lwu2+(2.d0+dsqrt(2.d0))*Lwu3+Lwu4)/6.d0
    phi4_wh = (Lwh1+(2.d0-dsqrt(2.d0))*Lwh2+(2.d0+dsqrt(2.d0))*Lwh3+Lwh4)/6.d0
    ...
    if(config_energy_conservation)then
        ...
        elseif(config_energy_conservation_scheme==2)then
            ...
            R2R4       = inner_product(Lwu2   ,Lwh2   ,Lwu4   ,Lwh4   )
            R3R4       = inner_product(Lwu3   ,Lwh3   ,Lwu4   ,Lwh4   )
            
            beta_n     = ((2.d0-dsqrt(2.d0))*R1R2 +(2.d0+dsqrt(2.d0))*((dsqrt(2.d0)-1.d0)*R1R3+(2.d0-dsqrt(2.d0))*R2R3)-dsqrt(2.d0)*R2R4+(2.d0+dsqrt(2.d0))*R3R4)/(6.d0*phi4_norm2)
            dtn        = beta_n*dt
        endif
    else
        dtn        = dt
