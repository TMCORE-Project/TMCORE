...
#ifndef NOLAPACK
  subroutine lapack_inv_matrix(n, A, B)

    integer, intent(in) :: n
    real(real_kind), intent(in ) :: A(n,n)
    real(real_kind), intent(out) :: B(n,n)

    real(real_kind) work(n)
    integer ipiv(n), ierr
    ...
    call dgetrf(n, n, B, n, ipiv, ierr)
    if (ierr /= 0) call log_error('math_inv_matrix: Matrix is numerically singular!')

    call dgetri(n, B, n, ipiv, work, n, ierr)
    if (ierr /= 0) call log_error('math_inv_matrix: Matrix inversion failed!')

  end subroutine lapack_inv_matrix
#endif
