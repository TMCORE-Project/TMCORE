...
!----------------------------------------------------------------------------
      ...
      do iCell = 1, nCells !  is this correct? - we need first halo cell also...
        ...
        advCells(1,iCell) = nReconstructCells
...
!         thetat(1) = 0.  !  this defines the x direction, cell center 1 -> 
        ...
        if (polynomial_order == 2) then
          ...
          do i=1,mw
            ...
            stop 'Unavaliable polynomial order, choose from 2,3,4'
        end if
        ...
        do i = 1, nEdgesOnCell(iCell)
          ...
          xec = xEdge(iEdge)/sphere_radius
          yec = yEdge(iEdge)/sphere_radius
          zec = zEdge(iEdge)/sphere_radius
         ...
         do i = 1, nEdgesOnCell(iCell)
           ...
           if (iCell == cellsOnEdge(1,iEdge)) then
             ...
             do j = 1, nReconstructCells
                deriv_two(j,1,iEdge) =  2.d0*cos2t    * bmatrix(4,j)  &
                                      + 2.d0*costsint * bmatrix(5,j)  &
                                      + 2.d0*sin2t    * bmatrix(6,j)
             end do
           else
             ...
             do j=1,nReconstructCells
               deriv_two(j,2,iEdge) =   2.d0*cos2t    * bmatrix(4,j)  &
                                      + 2.d0*costsint * bmatrix(5,j)  &
                                      + 2.d0*sin2t    * bmatrix(6,j)
             end do
           end if
